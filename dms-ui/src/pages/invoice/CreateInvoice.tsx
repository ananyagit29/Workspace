...
const CreateInvoice = () => {
  ...
  const handleInvoiceBlur = async () => {
    ...
    try {
      ...
      setInvoiceExists(!!res.data);
      if (res.data) showToast("Invoice number already exists", "error");
    } catch {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (invoiceExists) { showToast("Invoice number already exists", "error"); return; }
    if (!invoiceFile) { showToast("Please upload invoice PDF", "error"); return; }
    ...
    try {
      ...
      showToast(err.response?.data || "Failed to save invoice", "error");
    } finally {
      setSaving(false);
    }
  };
      ...
      <main style={{ flex: 1, padding: "8px 16px", overflowY: "auto" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <form onSubmit={handleSubmit}>
            <div style={cardStyle}>
              <div style={sectionTitle}>Invoice Information</div>
              <div style={fieldStyle}>
                <label style={labelStyle}>Invoice Number <span style={{ color: "#ef4444" }}>*</span></label>
                <input
                  value={invoiceNumber}
                  onChange={e => { setInvoiceNumber(e.target.value.toUpperCase()); setInvoiceExists(false); }}
                  onBlur={handleInvoiceBlur}
                  placeholder="Enter invoice number"
                  style={{ ...inputStyle, borderColor: invoiceExists ? "#ef4444" : "#e5e7eb" }}
                />
                <span style={{ fontSize: 10, color: invoiceExists ? "#ef4444" : "#9ca3af", marginTop: 2 }}>
                  {checkingInvoice ? "Validating invoice number..." : invoiceExists ? "This invoice number is already uploaded." : "Used for duplicate validation before saving."}
                </span>
              </div>
            </div>
            ...
            <div style={{ ...cardStyle, marginTop: 10 }}>
              ...
              <div style={{ fontSize: 10, color: "#9ca3af", marginBottom: 10 }}>Upload invoice in PDF format only. Maximum file size: 1 MB.</div>
              <div
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8, justifyContent: "flex-end" }}>
              ...
              <button type="submit" disabled={saving || invoiceExists || !invoiceFile} style={{
                ...primaryButton,
                background: saving || invoiceExists || !invoiceFile ? "#e5e7eb" : "#003366",
                color: saving || invoiceExists || !invoiceFile ? "#9ca3af" : "#fff",
                cursor: saving || invoiceExists || !invoiceFile ? "not-allowed" : "pointer",
              }}>
