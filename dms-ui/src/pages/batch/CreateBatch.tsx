...
interface FileEntry {
  subType: string;       // COA | INVOICE | IMAGE
  accept: string;
  label: string;
...
const CreateBatch = () => {
  ...
  // File entries — dynamic based on batchType
  ...
  // Reset downstream when batchType changes
  ...
  // Fetch product details on productCode blur
  const handleProductBlur = async () => {
    if (!productCode || !batchType) return;
    setFetchingProduct(true);
    setProductDetails(null);
    try {
      const res = await dmsApi.get("/batch/getProductDetails", {
        params: { batchType, productCode },
      });
      if (res.data && res.data.productName) {
        setProductDetails(res.data);
      } else {
        showToast("No matching product found", "error");
      }
    } catch {
  ...
  // Fetch existing files on batchNumber blur
  const handleBatchNumberBlur = async () => {
    if (!batchNumber || !productCode || !batchType) return;
    setFetchingFiles(true);
    try {
      const res = await dmsApi.get("/batch/getFileNames", {
        params: { productCode, batchType, batchNumber },
      });
      const data: { subApplicationName: string; fileName: string; filePath: string; hasFile: boolean }[] = res.data || [];
      buildFileEntries(batchType, data);
    } catch {
      buildFileEntries(batchType, []);
    } finally {
      setFetchingFiles(false);
    }
  };

  const buildFileEntries = (type: string, existing: { subApplicationName: string; fileName: string; filePath: string; hasFile: boolean }[]) => {
    ...
    if (type === "Third Party") {
      ...
      entries.push({
        subType: "COA", label: "COA", accept: ".pdf",
        existingFileName: coa?.hasFile ? coa.fileName : undefined,
        existingFilePath: coa?.hasFile ? coa.filePath : undefined,
        file: null,
      });
      entries.push({
        subType: "INVOICE", label: "Invoice", accept: ".pdf",
        existingFileName: invoice?.hasFile ? invoice.fileName : undefined,
        existingFilePath: invoice?.hasFile ? invoice.filePath : undefined,
        file: null,
      });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dmsApi.post("/batch/save", fd, {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        breadcrumb={`Dashboard › Batch Details › Create`}
        contextMeta={[
        ...
        onCreateClick={() => navigate("/batch")}
        createLabel="Search Batch"
      />
      ...
      <main style={{ flex: 1, padding: "8px 16px", overflow: "hidden" }}>
        <div style={{ maxWidth: 560, margin: "0 auto" }}>
          <form onSubmit={handleSubmit}>
            ...
            <div style={cardStyle}>
              ...
              {/* Batch Type + Batch Number in one row */}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 2fr", gap: 10 }}>
               ...
               <div style={fieldStyle}>
                  ...
                  <input value={batchNumber} onChange={e => setBatchNumber(e.target.value.toUpperCase())}
                    onBlur={handleBatchNumberBlur} disabled={!productDetails}
            ...
            {/* Upload Documents Card — shown after batch number is entered */}
