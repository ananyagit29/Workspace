...
const searchBatch = (params: Record<string, any>) => dmsApi.get("/batch/search", { params });
const loadFilterData = (locationId: string, type?: string) => dmsApi.get("/batch/loadFilterData", { params: { locationId, type } });
const removeBatchFile = (batchNumber: string, fileName: string, filePath: string) => dmsApi.delete("/batch/remove", { params: { batchNumber, fileName, filePath } });
...
const SearchBatch = () => {
  ...
  const toggleSelect = (filePath: string) => {
    setSelected(prev => {
      ...
      if (next.has(filePath)) {
        ...
        if (next.size >= 10) {
          showToast("Maximum 10 files can be selected", "error");
          return prev;
        }
        next.add(filePath);
      }
  ...
  const handleZipAndMail = async () => {
    ...
    try {
      await dmsApi.post("/batch/zipAndMail", {
        filePaths: Array.from(selected),
        recipients,
      }, { timeout: 60000 }); // ✅ 60s timeout for zip+mail
      showToast(`${selected.size} file(s) zipped and mailed to ${recipients.length} recipient(s)!`);
      setSelected(new Set());
      setRecipients([]);
      setMailTo("");
    } catch (err: any) {
      // If email was actually sent but timed out on frontend, show success
      if (err.code === "ECONNABORTED") {
        showToast("Request timed out — but email may have been sent. Please check your inbox.");
      } else {
        showToast("Failed to zip and mail files", "error");
      }
    }
  ...
  if (loading || !selections)
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="animate-spin rounded-full h-7 w-7 border-b-2 border-[#003366]" />
      </div>
    );
  if (!user) return null;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        onCreateClick={() => navigate("/batch/create")} 
          ...
          <div style={{ background: "#fff", borderRadius: 8, border: "1px solid #e5e7eb", padding: "8px 12px", marginBottom: 8 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 10 }}>
              ...
              <div>
                ...
                <input list="vendorList" value={vendorCode} onChange={e => setVendorCode(e.target.value.toUpperCase())}
              ...
              <div>
                ...
                <input list="productList" value={productCode} onChange={e => setProductCode(e.target.value.toUpperCase())}
                  placeholder="Type or select..."
                  style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 6, padding: "5px 8px", fontSize: 12, color: "#374151", background: "#f9fafb", outline: "none", boxSizing: "border-box" }} />
              ...
              <div>
                ...
                <input list="batchList" value={batchNumber} onChange={e => setBatchNumber(e.target.value.toUpperCase())}
                  placeholder="Type or select..."
                  style={{ width: "100%", border: "1px solid #e5e7eb", borderRadius: 6, padding: "5px 8px", fontSize: 12, color: "#374151", background: "#f9fafb", outline: "none", boxSizing: "border-box" }} />
            ...
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 8 }}>
              <button onClick={() => handleSearch(0)} disabled={searching}
                style={{ background: "#003366", color: "#fff", border: "none", borderRadius: 6, padding: "6px 24px", fontSize: 12, fontWeight: 500, cursor: "pointer", opacity: searching ? 0.6 : 1, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {hasSearched && (
            <div style={{ background: "#fff", borderRadius: 10, border: "1px solid #e5e7eb", overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 16px", background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                ...
                {results.length > 0 && (
                  <span style={{ fontSize: 11, color: "#9ca3af" }}>Page {currentPage + 1} of {totalPages}</span>
                )}
              </div>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      <thead>
                        <tr style={{ borderBottom: "1px solid #e5e7eb", background: "#f9fafb" }}>
                          {["", "Type", "Product Code", "Product Name", "Vendor Code", "Vendor Name", "Batch No.", "Sub Type", "File", "Uploaded By", "Date", ""].map((h, i) => (
                              ...
                              <td style={{ padding: "7px 8px", width: 32 }}>
                                {row.fileName && (
                                  <input type="checkbox"
                                    ...
                                    onChange={() => toggleSelect(row.filePath)}
      ...
      {selected.size > 0 && (
        <div style={{ position: "fixed", bottom: 32, left: "50%", transform: "translateX(-50%)", zIndex: 50, background: "#003366", borderRadius: 10, padding: "10px 16px", display: "flex", alignItems: "center", gap: 14, boxShadow: "0 4px 20px rgba(0,0,0,0.25)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 99, padding: "2px 10px", fontSize: 12, fontWeight: 700, color: "#fff" }}>
              {selected.size}/10
            </div>
          ...
          <button onClick={() => setSelected(new Set())}
            style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", background: "none", border: "none", cursor: "pointer", padding: 0 }}>
            Clear
          </button>
          <button onClick={() => setShowMailModal(true)} disabled={mailing}
            style={{ background: "#fff", color: "#003366", border: "none", borderRadius: 7, padding: "6px 16px", fontSize: 12, fontWeight: 600, cursor: mailing ? "not-allowed" : "pointer", opacity: mailing ? 0.7 : 1, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      {showMailModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center" }}
          ...
          <div style={{ background: "#fff", borderRadius: 12, padding: "20px 24px", width: 380, boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}
                ...
                {recipients.map((r, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 5, background: "#e0e7ff", borderRadius: 99, padding: "3px 8px 3px 10px", fontSize: 11, color: "#3730a3" }}>
                    ...
                    <button onClick={() => setRecipients(prev => prev.filter((_, j) => j !== i))}
                      style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontSize: 12, color: "#6366f1", lineHeight: 1 }}>✕</button>
                  </div>
                ))}
              </div>
            )}
