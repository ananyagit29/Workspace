...
const TruckLoadStuffLayout = () => {
  ...
  const tabs = (
    <>
      <TabLink to="/truck-load-stuff" end label="Search" />
      {canCreate && <TabLink to="/truck-load-stuff/create" label="Create" />}
    </>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        pageTitle="Truck Load Stuff"
        breadcrumb={`Dashboard > Truck Load Stuff${selections.subApp ? ` > ${selections.subApp}` : ""}`}
        contextMeta={[
...
export default TruckLoadStuffLayout;
