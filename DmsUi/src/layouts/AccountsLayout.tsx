...
const AccountsLayout = () => {
  ...
  const tabs = (
    <>
      <TabLink to="/accounts" end label="Search" />
      {canCreate && <TabLink to="/accounts/create" label="Create" />}
      {canReport && <TabLink to="/accounts/report" label="Report" />}
    </>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        pageTitle="Accounts"
        breadcrumb={`Dashboard › Accounts${selections.subApp ? ` › ${selections.subApp}` : ""}`}
        contextMeta={[
