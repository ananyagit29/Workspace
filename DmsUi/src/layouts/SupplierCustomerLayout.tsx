...
const SupplierCustomerLayout = () => {
  const { user, loading: authLoading } = useContext(AuthContext);
  const navigate = useNavigate();
  const { canCreate, loading: rightsLoading } = useAppRights();
  ...
  const tabs = (
    <>
      <TabLink to="/supplier-customer" end label="Search" />
      {canCreate && (
        <>
          <TabLink to="/supplier-customer/create" label="Create" />
          <TabLink to="/supplier-customer/add-files" label="Add Files" />
        </>
      )}
    </>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        applicationName="SUPPLIER_AND_CUSTOMER"
        pageTitle="Supplier & Customer"
        breadcrumb="Dashboard › Supplier & Customer"
        contextMeta={[
...
export default SupplierCustomerLayout;
