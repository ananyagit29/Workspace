...
const ServiceAgreementLayout = () => {
  ...
  const tabs = (
    <>
      <TabLink to="/service-agreement" end label="Search" />
      {canCreate && <TabLink to="/service-agreement/create" label="Create" />}
    </>
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "#f3f4f6", overflow: "hidden" }}>
      <Header
        ...
        breadcrumb={`Dashboard › Service Agreement${selections.subApp ? ` › ${selections.subApp}` : ""}`}
        contextMeta={[
